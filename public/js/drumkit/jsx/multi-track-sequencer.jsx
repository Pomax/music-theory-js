...
class MultiTrackSequencer extends Component {
    constructor(props) {
        ...
        Object.keys(instruments).map(name => {
            let track = <Track ref={e => (track.api=e)} name={name} instrument={instruments[name]} stepCount={stepCount} ticksPerStep={ticksPerStep} volume={this.gainNode} />
    ...
    render() {
        return (
            <div className={"multi-track-sequencer" + (this.props.collapsed ? " collapsed" : "")}>
                { this.volume }
                { this.state.tracks }
            </div>
        );
