...
class MIDIListener {
    constructor(eventType) {
        ...
        this.channels = { omni: [] };
        for (let i=0; i<16; i++) { this.channels[i] = []; }
    }

    addListener(listener, channel) {
        if (!listener[`on${this.eventType}`]) {
            throw new Error(`Listener does not implement an on${this.eventType} function.`);
        }
    ...
    receive(channel, ...data) {
        ...
        this.channels.omni.forEach(l => l[`on${this.eventType}`](...data, channel));
    }
}
